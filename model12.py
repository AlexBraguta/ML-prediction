...
# We'll define a global param_dist that can be adjusted in case of over/underfitting
param_dist_lr = {
...
def hyperparameter_optimization(x_train_balanced, y_train_balanced):
    global param_dist_lr
    # Print a single message before fitting
    print("Starting hyperparameter optimization with current param_dist...")
...
def final_prints(ensemble_model, x_test, y_test, y_pred, y_pred_proba, train_accuracy, test_accuracy, x_scaled, x):
    ...
    # Fit status
    print("Model fit: The model is Well fitted.")
...
def main():
    ...
    # We'll do a loop until well fitted or until we tried up to 3 times
    max_attempts = 3
    attempt = 0
    ...
    while True:
        ...
        if fit_status == "well fitted" or attempt == max_attempts:
            ...
            final_prints(ensemble, x_test, y_test, y_pred, y_pred_proba, train_acc, test_acc, x_scaled, X)
            break
        else:
            # If overfitted, let's reduce complexity
            if fit_status == "overfitted":
                print("Overfitted detected. Adjusting param_dist to reduce complexity.")
            ...
            if fit_status == "underfitted":
                print("Underfitted detected. Adjusting param_dist to allow more complexity.")
                if None not in param_dist['max_depth']:
                    param_dist['max_depth'].append(None)
                # ensure we have bigger n_estimators
                if 500 not in param_dist['n_estimators']:
                    param_dist['n_estimators'].append(500)
            print("Re-running with updated param_dist...\n")
